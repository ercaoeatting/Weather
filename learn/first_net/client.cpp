...
int main(int argc, char *argv[]) {
    ...
    // memcpy(&servaddr.sin_addr, gethostbyname(argv[1])->h_addr, h->h_length); //
    // 指定服务端的IP地址。 servaddr.sin_port = htons(atoi(argv[2])); // 指定服务端的通信端口。
    ...
    char buffer[1024];
    for (int ii = 0; ii < 30000; ii++) // 循环3次，将与服务端进行三次通讯。
    {
        int iret;

...
bool replacestr(string &str, string const &str1, string const &str2, bool bloop)
{
...
bool replacestr(char *str, string const &str1, string const &str2, bool bloop)
{
    if (str == nullptr) {
        return false;
    }
...
char *picknumber(string const &src, char *dest, bool const bsigned,
                 bool const bdot)
{
    if (dest == nullptr) {
...
string &picknumber(string const &src, string &dest, bool const bsigned,
                   bool const bdot)
{
...
string picknumber(string const &src, bool const bsigned, bool const bdot)
{
    string dest;
...
bool matchstr(string const &str, string const &rules)
{
    ...
    for (ii = 0; ii < cmdstr.size(); ii++) {
        ...
        for (jj = 0; jj < cmdsubstr.size(); jj++) {
            // 如果是文件名的首部
            if (jj == 0) {
                if (filename.substr(0, cmdsubstr[jj].length()) !=
                    cmdsubstr[jj]) {
                    break;
                }
            }
...
ccmdstr::ccmdstr(string const &buffer, string const &sepstr,
                 bool const bdelspace)
{
...
void ccmdstr::splittocmd(string const &buffer, string const &sepstr,
                         bool const bdelspace)
{
    // 清除所有的旧数据
    m_cmdstr.clear();
           ...
           string::npos) // 从pos的位置开始，查找下一个分隔符的位置。
    {
        ...
        pos =
            pos1 + sepstr.length(); // 下次从buffer中查找分隔符的起始位置后移。
    }
...
bool ccmdstr::getvalue(int const ii, string &value, int const ilen) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, char *value, int const len) const
{
...
bool ccmdstr::getvalue(int const ii, int &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
    ...
    try {
        value =
            stoi(picknumber(m_cmdstr[ii], true)); // stoi有异常，需要处理异常。
...
bool ccmdstr::getvalue(int const ii, unsigned int &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
    ...
    try {
        value = stoi(picknumber(
            m_cmdstr[ii])); // stoi有异常，需要处理异常。不提取符号 + -
    } catch (std::exception const &e) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, long &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
    ...
    try {
        value =
            stol(picknumber(m_cmdstr[ii], true)); // stol有异常，需要处理异常。
    } catch (std::exception const &e) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, unsigned long &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
    ...
    try {
        value = stoul(picknumber(
            m_cmdstr[ii])); // stoul有异常，需要处理异常。不提取符号 + -
    } catch (std::exception const &e) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, double &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, float &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
...
bool ccmdstr::getvalue(int const ii, bool &value) const
{
    if (ii >= m_cmdstr.size()) {
        return false;
    }
...
ostream &operator<<(ostream &out, ccmdstr const &cmdstr)
{
...
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname,
                  string &value, int const ilen)
{
...
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, char *value,
                  int const len)
{
    if (value == nullptr) {
        return false;
                  ...
                  unsigned int &value)
{
    ...
    try {
        value =
            stoi(picknumber(str)); // stoi有异常，需要处理异常。不提取符号 + -
    } catch (std::exception const &e) {
        return false;
    }
...
string &timetostr(time_t const ttime, string &strtime, string const &fmt)
{
    ...
    if (fmt == "hh24miss") {
        strtime =
            sformat("%02u%02u%02u", sttm.tm_hour, sttm.tm_min, sttm.tm_sec);
        return strtime;
    }
...
string &ltime(string &strtime, string const &fmt, int const timetvl)
{
    time_t timer;
    time(&timer);            // 获取系统当前时间。
...
char *ltime(char *strtime, string const &fmt, int const timetvl)
{
    if (strtime == nullptr) {
        return nullptr; // 判断空指针。
    }
...
string ltime1(string const &fmt, int const timetvl)
{
    string strtime;
...
bool clogfile::open(string const &filename, ios::openmode const mode,
                    bool const bbackup, bool const benbuffer)
{
    // 如果日志文件是打开的状态，先关闭它。
    if (fout.is_open()) {
        fout.close();
...
bool clogfile::backup()
{
    ...
    if (fout.tellp() > m_maxsize * 1024 * 1024) {
        ...
        rename(m_filename.c_str(), bak_filename.c_str()); // back
...
bool cifile::open(string const &filename, ios::openmode const mode)
{
    // 如果文件是打开的状态，先关闭它。
    if (fin.is_open()) {
        fin.close();
...
int cifile::read(void *buf, int const bufsize)
{
...
bool cifile::readline(string &buf, string const &endbz)
{
    buf.clear();    // 清空buf。
    ...
    while (true) {
        ...
        if (endbz == "") {
            ...
            if (buf.find(endbz, buf.length() - endbz.length()) !=
                string::npos) {
                return true;
            }
        }
...
bool cofile::open(string const &filename, bool const btmp,
                  ios::openmode const mode, bool const benbuffer)
{
    // 如果文件是打开的状态，先关闭它。
    if (fout.is_open()) {
        fout.close();
...
bool newdir(string const &pathorfilename, bool bisfilename)
{
...
bool addtime(string const &in_stime, string &out_stime, int const timetvl,
             string const &fmt)
{
    time_t timer;
...
bool addtime(string const &in_stime, char *out_stime, int const timetvl,
             string const &fmt)
{
...
bool filemtime(string const &filename, string &mtime, string const &fmt)
{
    struct stat st_filestat; // 存放文件信息的结构体。
...
bool filemtime(string const &filename, char *mtime, string const &fmt)
{
    struct stat st_filestat; // 存放文件信息的结构体。
...
void cdir::setfmt(string const &fmt)
{
    m_fmt = fmt;
}

bool cdir::opendir(string const &dirname, string const &rules,
                   int const maxfiles, bool const bandchild, bool bsort)
{
...
bool cdir::_opendir(string const &dirname, string const &rules,
                    int const maxfiles, bool const bandchild)
{
    DIR *dir; // 目录指针。
...
bool ctcpclient::connect(string const &ip, int const port)
{
    ...
    if (::connect(m_connfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) !=
        0) {
        ::close(m_connfd);
        m_connfd = -1;
        return false;
...
bool ctcpserver::initserver(unsigned int const port, int const backlog)
{
...
bool ctcpserver::read(void *buffer, int const ibuflen,
                      int const itimeout) // 接收二进制数据。
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpserver::read(string &buffer, int const itimeout) // 接收文本数据。
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpclient::read(void *buffer, int const ibuflen,
                      int const itimeout) // 接收二进制数据。
{
    if (m_connfd == -1) {
        return false;
...
bool ctcpclient::read(string &buffer, int const itimeout) // 接收文本数据。
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpserver::write(void const *buffer,
                       int const ibuflen) // 发送二进制数据。
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpserver::write(string const &buffer)
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpclient::write(void const *buffer, int const ibuflen)
{
    if (m_connfd == -1) {
        return false;
    }
...
bool ctcpclient::write(string const &buffer)
{
    if (m_connfd == -1) {
        return false;
    }
...
bool tcpread(int const sockfd, string &buffer,
             int const itimeout) // 接收文本数据。
{
    if (sockfd == -1) {
        return false;
    }
...
bool tcpwrite(int const sockfd, void const *buffer,
              int const ibuflen) // 发送二进制数据。
{
    if (sockfd == -1) {
        return false;
...
bool tcpwrite(int const sockfd, string const &buffer) // 发送文本数据。
{
    if (sockfd == -1) {
        return false;
    }
...
// 把当前进程的信息加入共享内存进程组中。
bool cpactive::addpinfo(int const timeout, string const &pname,
                        clogfile *logfile)
{
    if (m_pos != -1) {
        return true;

...
// str：待处理的字符串。
// cc：需要删除的字符，缺省删除空格。
char *deletelchr(char *str, int const cc = ' ');
string &deletelchr(string &str, int const cc = ' ');

// 删除字符串右边指定的字符。
// str：待处理的字符串。
// cc：需要删除的字符，缺省删除空格。
char *deleterchr(char *str, int const cc = ' ');
string &deleterchr(string &str, int const cc = ' ');

// 删除字符串左右两边指定的字符。
// str：待处理的字符串。
// chr：需要删除的字符，缺省删除空格。
char *deletelrchr(char *str, int const cc = ' ');
string &deletelrchr(string &str, int const cc = ' ');
...
bool replacestr(char *str, string const &str1, string const &str2, bool const bloop = false);
bool replacestr(string &str, string const &str1, string const &str2, bool const bloop = false);
...
// 注意：src和dest可以是同一个变量。
char *picknumber(string const &src, char *dest, bool const bsigned = false, bool const bdot = false);
string &picknumber(string const &src, string &dest, bool const bsigned = false, bool const bdot = false);
string picknumber(string const &src, bool const bsigned = false, bool const bdot = false);
...
bool matchstr(string const &str, string const &rules);
...
private:
    ...
    ccmdstr(ccmdstr const &) = delete;            // 禁用拷贝构造函数。
    ccmdstr &operator=(ccmdstr const &) = delete; // 禁用赋值函数。
public:
    ccmdstr() {}                                  // 构造函数。

    ccmdstr(string const &buffer, string const &sepstr, bool const bdelspace = false);

    string const &operator[](int ii) const // 重载[]运算符，可以像访问数组一样访问m_cmdstr成员。
    {
        return m_cmdstr[ii];
    }
    ...
    void splittocmd(string const &buffer, string const &sepstr, bool const bdelspace = false);
    ...
    bool getvalue(int const ii, string &value,
                  int const ilen = 0) const; // C++风格字符串。视频中没有第三个参数，加上第三个参数更好。
    bool getvalue(int const ii, char *value,
                  int const ilen = 0) const;                 // C风格字符串，ilen缺省值为0-全部长度。
    bool getvalue(int const ii, int &value) const;           // int整数。
    bool getvalue(int const ii, unsigned int &value) const;  // unsigned int整数。
    bool getvalue(int const ii, long &value) const;          // long整数。
    bool getvalue(int const ii, unsigned long &value) const; // unsigned long整数。
    bool getvalue(int const ii, double &value) const;        // 双精度double。
    bool getvalue(int const ii, float &value) const;         // 单精度float。
    bool getvalue(int const ii, bool &value) const;          // bool型。

    ~ccmdstr();                                              // 析构函数。
};

// 重载<<运算符，输出ccmdstr::m_cmdstr中的内容，方便调试。
ostream &operator<<(ostream &out, ccmdstr const &cc);
///////////////////////////////////// /////////////////////////////////////
...
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, string &value,
                  int const ilen = 0); // 视频中没有第三个参数，加上第三个参数更好。
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, char *value, int const ilen = 0);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, bool &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, int &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, unsigned int &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, long &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, unsigned long &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, double &value);
bool getxmlbuffer(string const &xmlbuffer, string const &fieldname, float &value);

///////////////////////////////////// /////////////////////////////////////

// C++格式化输出函数模板。
template <typename... Args>
bool sformat(string &str, char const *fmt, Args... args)
{
...
template <typename... Args>
string sformat(char const *fmt, Args... args)
{
    string str;
...
*/
string &ltime(string &strtime, string const &fmt = "", int const timetvl = 0);
char *ltime(char *strtime, string const &fmt = "", int const timetvl = 0);
// 为了避免重载的岐义，增加ltime1()函数。
string ltime1(string const &fmt = "", int const timetvl = 0);

// 把整数表示的时间转换为字符串表示的时间。
// ttime：整数表示的时间。
// strtime：字符串表示的时间。
// fmt：输出字符串时间strtime的格式，与ltime()函数的fmt参数相同，如果fmt的格式不正确，strtime将为空。
string &timetostr(time_t const ttime, string &strtime, string const &fmt = "");
char *timetostr(time_t const ttime, char *strtime, string const &fmt = "");
// 为了避免重载的岐义，增加timetostr1()函数。
string timetostr1(time_t const ttime, string const &fmt = "");

// 把字符串表示的时间转换为整数表示的时间。
// strtime：字符串表示的时间，格式不限，但一定要包括yyyymmddhh24miss，一个都不能少，顺序也不能变。
// 返回值：整数表示的时间，如果strtime的格式不正确，返回-1。
time_t strtotime(string const &strtime);
...
bool addtime(string const &in_stime, char *out_stime, int const timetvl, string const &fmt = "");
bool addtime(string const &in_stime, string &out_stime, int const timetvl, string const &fmt = "");
...
bool newdir(string const &pathorfilename, bool bisfilename = true);
...
bool renamefile(string const &srcfilename, string const &dstfilename);
...
bool copyfile(string const &srcfilename, string const &dstfilename);

// 获取文件的大小。
// filename：待获取的文件名，建议采用绝对路径的文件名。
// 返回值：如果文件不存在或没有访问权限，返回-1，成功返回文件的大小，单位是字节。
int filesize(string const &filename);
...
bool filemtime(string const &filename, char *mtime, string const &fmt = "yyyymmddhh24miss");
bool filemtime(string const &filename, string &mtime, string const &fmt = "yyyymmddhh24miss");

// 重置文件的修改时间属性。
// filename：待重置的文件名，建议采用绝对路径的文件名。
// mtime：字符串表示的时间，格式不限，但一定要包括yyyymmddhh24miss，一个都不能少，顺序也不能变。
// 返回值：true-成功；false-失败，失败的原因保存在errno中。
bool setmtime(string const &filename, string const &mtime);
...
private:
    ...
    cdir(cdir const &) = delete;            // 禁用拷贝构造函数。
    cdir &operator=(cdir const &) = delete; // 禁用赋值函数。
public:
    ...
    bool opendir(string const &dirname, string const &rules, int const maxfiles = 10000, bool const bandchild = false,
                 bool bsort = false);

private:
    // 这是一个递归函数，被opendir()的调用，在cdir类的外部不需要调用它。
    bool _opendir(string const &dirname, string const &rules, int const maxfiles, bool const bandchild);
...
public:
    ...
    // mode，打开文件的模式。
    // benbuffer，是否启用文件缓冲区。
    bool open(string const &filename, bool const btmp = true, ios::openmode const mode = ios::out,
              bool const benbuffer = true);

    // // 把数据以文本的方式格式化输出到文件。
    // template <typename... Args>
    // bool writeline(char const *fmt, Args... args)
    // {
    ...
    // C++20的重载版本
    template <typename... Args>
    bool writeline(std::format_string<Args...> fmt, Args &&...args)
    {
        if (!fout.is_open()) {
            return false;
        }
        fout << std::format(fmt, std::forward<Args>(args)...);
        return fout.good();
    }
...
public:
    ...
    // filename，待打开的文件名。
    // mode，打开文件的模式。
    bool open(string const &filename, ios::openmode const mode = ios::in);

    // 以行的方式读取文本文件，endbz指定行的结尾标志，缺省为空，没有结尾标志。
    bool readline(string &buf, string const &endbz = "");

    // 读取二进制文件，返回实际读取到的字节数。
    int read(void *buf, int const bufsize);
...
private:
    ...
    spinlock_mutex(spinlock_mutex const &) = delete;
    spinlock_mutex &operator=(spinlock_mutex const) = delete;
...
public:
    ...
    // 3）如果业务无法容忍，可以用信号量加锁。
    bool open(string const &filename, ios::openmode const mode = ios::app, bool const bbackup = true,
              bool const benbuffer = false);

    // 把日志内容以文本的方式格式化输出到日志文件，并且，在日志内容前面写入时间。
    template <typename... Args>
    bool write(char const *fmt, Args... args)
    {
        if (fout.is_open() == false) {
            return false;
        }
        ...
        m_splock.lock();                                           // 加锁。
        fout << ltime1() << " "
             << std::vformat(fmt, std::make_format_args(args...)); // 把当前时间和日志内容写入日志文件。
        m_splock.unlock();                                         // 解锁。
    ...
    // 注意：内容换行用\n，不能用endl。
    template <typename T>
    clogfile &operator<<(T const &value)
    {
        m_splock.lock();
        fout << value;
        m_splock.unlock();
...
public:
    ...
    // port：服务端通讯的端口。
    // 返回值：true-成功；false-失败。
    bool connect(string const &ip, int const port);
    ...
    bool read(string &buffer, int const itimeout = 0);                  // 接收文本数据。
    bool read(void *buffer, int const ibuflen, int const itimeout = 0); // 接收二进制数据。

    // 向对端发送数据。
    // buffer：待发送数据缓冲区。
    // ibuflen：待发送数据的大小。
    // 返回值：true-成功；false-失败，如果失败，表示socket连接已不可用。
    bool write(string const &buffer);                  // 发送文本数据。
    bool write(void const *buffer, int const ibuflen); // 发送二进制数据。
...
public:
    ...
    bool initserver(unsigned int const port, int const backlog = 5);
...
// 返回值：true-成功；false-失败，失败有两种情况：1）等待超时；2）socket连接已不可用。
bool tcpread(int const sockfd, string &buffer, int const itimeout = 0); // 读取文本数据。
bool tcpread(int const sockfd, void *buffer, int const ibuflen,
             int const itimeout = 0);                                   // 读取二进制数据。
...
// ibuflen：待发送数据的字节数。
// 返回值：true-成功；false-失败，如果失败，表示socket连接已不可用。
bool tcpwrite(int const sockfd, string const &buffer);                  // 写入文本数据。
bool tcpwrite(int const sockfd, void const *buffer, int const ibuflen); // 写入二进制数据。
...
bool readn(int const sockfd, char *buffer, size_t const n);
...
bool writen(int const sockfd, char const *buffer, size_t const n);
...
private:
    ...
    squeue(squeue const &) = delete;            // 禁用拷贝构造函数。
    squeue &operator=(squeue const &) = delete; // 禁用赋值函数。
public:
    squeue()
    {
        init();
...
private:
    ...
    csemp(csemp const &) = delete;            // 禁用拷贝构造函数。
    csemp &operator=(csemp const &) = delete; // 禁用赋值函数。
public:
    csemp() : m_semid(-1) {}
...
struct st_procinfo {
    ...
    st_procinfo(int const in_pid, string const &in_pname, int const in_timeout, time_t const in_atime)
        : pid(in_pid),
          timeout(in_timeout),
          atime(in_atime)
    {
...
public:
    ...
    // 把当前进程的信息加入共享内存进程组中。
    bool addpinfo(int const timeout, string const &pname = "", clogfile *logfile = nullptr);
